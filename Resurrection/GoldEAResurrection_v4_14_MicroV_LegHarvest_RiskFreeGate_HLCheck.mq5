...
void ActivateLeg(int dir, double structProtected, double actualSL)
{
   ...
   if(InpVerboseLogs) Print("LEG ENTRY COUNT: ",g_legEntries," / ",InpMaxEntriesPerLeg);
}
...
bool LegReentryBuy(string &why)
{
   if(!g_legActive || g_legDir!=1) { why="leg not active (buy)"; return false; }
   if(g_legEntries >= InpMaxEntriesPerLeg) { why="leg entry cap reached"; return false; }
   ...
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   if(bid <= pullLo + InpReentryHLBufferPoints*_Point) { why="price not above last HL buffer"; return false; }
   ...
   why="LEG REENTRY BUY: OK";
   return true;
}

bool LegReentrySell(string &why)
{
   ...
   double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
   if(ask >= pullHi - InpReentryHLBufferPoints*_Point) { why="price not below last LH buffer"; return false; }
...
if(g_lastTradeTime>0 && (TimeCurrent()-g_lastTradeTime) < InpTradeCooldownSeconds)
{
   ...
   {
      string lw="";
      if(LegReentryBuy(lw)) { sigBuy=true; whyB=lw; protLL=LowestN(InpStructLookback, 3); targHH=HighestN(InpStructLookback, 3); }
      else if(LegReentrySell(lw)) { sigSell=true; whyS=lw; protHH=HighestN(InpStructLookback, 3); targLL=LowestN(InpStructLookback, 3); }
   }
