...
int BuildKeyLevels(KeyLevel &levels[])
{
   ...
   KeyLevel k;
   k.type=K_WHI; k.low=iHigh(_Symbol, PERIOD_W1, 1); k.high=k.low; ArrayPush(levels,k);
   k.type=K_WLO; k.low=iLow (_Symbol, PERIOD_W1, 1); k.high=k.low; ArrayPush(levels,k);

   // Monday Open
   int monShift = GetMondayDailyShift();
   if(monShift>=0)
   {
      k.type=K_MON; k.low=iOpen(_Symbol, PERIOD_D1, monShift); k.high=k.low; ArrayPush(levels,k);
   }

   // PDH / PDL (yesterday, shift 1)
   k.type=K_PDH; k.low=iHigh(_Symbol, PERIOD_D1, 1); k.high=k.low; ArrayPush(levels,k);
   k.type=K_PDL; k.low=iLow (_Symbol, PERIOD_D1, 1); k.high=k.low; ArrayPush(levels,k);

   // Daily open (today, shift 0)
   k.type=K_DOPEN; k.low=iOpen(_Symbol, PERIOD_D1, 0); k.high=k.low; ArrayPush(levels,k);
...
void OnTick()
{
   ...
   if(dailyBias==BIAS_BUY)
      ok = trade.Buy(lot, _Symbol, 0.0, sl, 0.0, "DowScalpPro BUY");
   else if(dailyBias==BIAS_SELL)
      ok = trade.Sell(lot, _Symbol, 0.0, sl, 0.0, "DowScalpPro SELL");
